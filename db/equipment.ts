...
export async function listEquipment(options: {
  ...
  if (!process.env.DATABASE_URL && process.env.NODE_ENV !== 'production') {
    const preview = seedEquipment.filter((item) => !options.featuredOnly || item.featured);
    return options.limit ? preview.slice(0, options.limit) : preview;
  }
...
export async function getEquipmentRecord(slug: string, publishedOnly = true): Promise<EquipmentItem | null> {
  if (!process.env.DATABASE_URL && process.env.NODE_ENV !== 'production') {
    return seedEquipment.find((item) => item.slug === slug) ?? null;
  }
  await initializeInventory();
